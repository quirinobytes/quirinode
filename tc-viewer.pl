        ...
        for (@TC) {
                if ($hfsc eq "1") {
                        ...
                        if(/^class htb (\S+) (root|parent (\S+)) .*rate (\S+) ceil (\S+) /) {
                                ...
                                $class .= "0" if ($class =~ /:$/);
                                $parent .= "0" if ($parent =~ /:$/);
                if (substr($pom4,0,-3) =~  'K'){
                    $pom4 = substr($pom4,0,-4)/8;
                    }
                    else {
                    $pom4 = substr($pom4,0,-4)/800;
                    }

                if (substr($pom5,0,-3) =~  'K'){
                    $pom5 = substr($pom5,0,-4)/8;
                    }
                    else {
                    $pom5 = substr($pom5,0,-4)/800;
                    }
...
while (1) {
    ...
    printf " ## $colors{$color_set{'Neutral'}} $colors{'YEL'} $hostname ";
        print "\n\n\tMode: ", $hfsc eq "0" ? "HTB" : "HFSC" ,"   $colors{'GRE'} @ $iface $colors{$color_set{'Reset'}}\n\n";
